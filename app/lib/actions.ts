...
export async function validateTicket(ticketCode: any) {
  try {
    ...
    const response = await axios.post(
      `http://pocki-api-env-1.eba-pprtwpab.us-east-1.elasticbeanstalk.com/api/taquilla/validateTicketNew`,
      ticketCode,
...
export async function updateCandidato(
  ...
  try {
    ...
    const token = session?.accessToken;
    const response = await axios.post(
      `http://pocki-api-env-1.eba-pprtwpab.us-east-1.elasticbeanstalk.com/api/taquilla/updateUserByIdTaquilla`,
      {
        id: id,
        updates: {
          name: nombre,
